...
> 과정
1. SYN ( Synchronization ) 패킷 전송 : 클라이언트 -> 서버
- 클라이언트는 ISN(Initial Sequence Number)을 생성하여 클라이언트 측 TCP 소켓에 저장 
- 클라이언트는 서버와 연결을 시작하기 위해 SYN flag가 설정된 SYN 패킷에 ISN을 담아 서버로 전송
- 이후 클라이언트는 서버로부터 SYN-ACK 패킷이 올때까지 자신의 ISN과 서버의 응답 기다림 
- SYN flag : 클라이언트가 서버에 "연결을 시작하고 싶다"는 신호
2. SYN-ACK ( Acknowledgment ) : 서버 -> 클라이언트
- 서버는 클라이언트의 SYN 요청을 받고, 클라이언트의 ISN을 서버측 TCP 소켓에 저장.
- 서버는 자신의 ISN 값 생성하고, 클라이언트에게 전송하기 위해 SYN-ACK 패킷에 포함
- 클라이언트가 보낸 ISN + 1 값을 ACK 번호로 설정해, 클라이언트에 응답 
- SYN flag : 서버가 "연결을 시작하겠다" 는 신호 
- ACK flag : 클라이언트가 보낸 SYN을 받았다는 신호
3. ACK : 클라이언트 -> 서버
- 클라이언트는 서버의 SYN-ACK 패킷을 받고, 이를 확인하는 ACK flag가 설정된 패킷을 서버로 전송
- 클라이언트는 서버의 ISN + 1 값을 ACK 번호로 포함하여 응답
- 클라이언트는 서버의 ISN을 저장하고, 이 정보를 기반으로 이후 데이터 주고받음
- ACK flag : 연결이 성공적으로 설정되었음을 서버에 전송 

> ISN을 패킷에 포함하여 보내는 이유
- ISN (Initial Sequence Number): TCP 연결에서 처음으로 전송되는 데이터의 순서 번호, 랜덤 값 
- TCP 연결에서 데이터의 순서와 무결성을 보장하기 위해
