...
- HTTPS는 TLS또는 SSL을 추가한 프로토콜로, 데이터를 암호화하여 클라이언트와 서버 간 통신 보호
- 암호화
  + 클라이언트와 서버 간의 데이터는 암호화되어 전송되므로, 제3자가 데이터를 도청해도 내용을 알 수 없음
- 데이터 무결성
  + 전송 중 데이터가 변조되었는지 확인하는 기능으로, 송신자와 수신자 간의 데이터를 안전하게 전송
- 인증
  + HTTPS는 클라이언트가 통신하는 서버가 신뢰할 수 있는 서버임을 보장하는 디지털 인증서를 통해 서버의 신원을 확인

## TLS/SSL 핸드셰이크 과정은 어떻게 이루어지나요?
- TLS/SSL 핸드셰이크 : 클라이언트와 서버 간의 안전한 통신 채널을 설정하기 위한 과정
- 클라이언트 헬로(Client Hello): 클라이언트가 지원하는 암호화 방식(Cipher Suite)과 TLS/SSL 버전, 난수를 서버에 보냅니다.
- 서버 헬로(Server Hello): 서버는 클라이언트의 암호화 방식 중 하나를 선택하고, 디지털 인증서(공개키 포함)와 난수를 클라이언트에 보냅니다.
- 인증서 검증: 클라이언트는 서버의 디지털 인증서를 확인하고, 신뢰할 수 있는 인증 기관(CA)에서 발급된 인증서인지 확인합니다.
- 프리마스터 시크릿 전송: 클라이언트는 서버의 공개키로 프리마스터 시크릿을 암호화하여 서버로 전송합니다.
- 세션 키 생성: 클라이언트와 서버는 각자의 난수와 프리마스터 시크릿을 기반으로 세션 키를 생성하고, 이 키로 이후의 데이터를 암호화하여 통신합니다.
- 완료 메시지 전송: 클라이언트와 서버는 암호화된 상태에서 완료 메시지를 서로 교환하고, 안전한 통신이 시작됩니다.

## MITM(중간자 공격, Man-In-The-Middle)이란 무엇이며, 이를 방어하는 방법은 무엇인가요?
> MITM(중간자 공격, Man-In-The-Middle)
- 공격자가 클라이언트와 서버 간의 통신을 가로채고 중간에서 데이터를 훔치거나 변조하는 공격

> 방어 방법
- HTTPS 사용: HTTPS는 데이터를 암호화하여 제3자가 데이터를 가로채더라도 이를 읽을 수 없도록 합니다.
- 디지털 인증서 사용: 인증서를 통해 서버의 신원을 검증하고, 중간자가 가짜 서버를 운영하는 것을 방지합니다.
- HSTS(HTTP Strict Transport Security): 클라이언트가 항상 HTTPS로 서버에 연결되도록 하여, HTTP로의 다운그레이드를 방지합니다.
- DNSSEC: DNS 서버의 응답을 검증하여, 공격자가 DNS 요청을 조작하지 못하도록 합니다.
...
> 방어 방법
- WAF(Web Application Firewall): 공격 트래픽을 필터링하여 비정상적인 요청을 차단합니다.
- 트래픽 모니터링 및 자동 차단: 트래픽 양을 모니터링하고, 비정상적인 트래픽 발생 시 이를 자동으로 차단하는 시스템을 도입합니다.
- CDN(Content Delivery Network): CDN을 사용해 트래픽을 분산시키고, 공격이 발생할 경우 여러 서버로 트래픽을 나누어 대응합니다.
- Rate Limiting: IP당 요청 수를 제한하여 과도한 요청을 방지합니다.
- 클라우드 기반 보호 서비스: AWS Shield, Cloudflare 등과 같은 클라우드 기반 DDoS 방어 서비스를 활용해 공격을 차단합니다.


## CORS는 어떻게 웹 보안에 기여하나요?
> CORS(Cross-Origin Resource Sharing)
- 웹 브라우저가 다른 도메인에서 리소스를 요청할 때, 해당 요청이 허용되었는지 확인하는 보안 기능
- 외부 리소스 제어: CORS 정책은 서버가 명시적으로 허용한 도메인에서만 리소스를 요청할 수 있도록 합니다.
- 악의적 요청 방지: XSS(Cross-Site Scripting)나 CSRF(Cross-Site Request Forgery) 공격을 방지할 수 있습니다.
- 보안 정책 구현: 서버가 클라이언트의 요청 헤더에 포함된 Origin을 확인하고, 요청을 허용할지 결정합니다.

 
## 네트워크 방화벽의 역할은 무엇인가요?
> 네트워크 방화벽
- 네트워크 내부와 외부 간의 트래픽을 모니터링하고 필터링하는 보안 시스템입니다. 방화벽은 설정된 규칙에 따라 허용되거나 차단된 트래픽을 제어하여, 외부로부터의 공격이나 내부 정보 유출을 방지
- 트래픽 필터링: 특정 IP 주소, 포트, 프로토콜을 기반으로 트래픽을 허용하거나 차단합니다.
- 네트워크 분리: 외부 네트워크(인터넷)와 내부 네트워크(사설 네트워크)를 분리하여 외부 공격을 차단합니다.
- 침입 방지: 미리 설정된 보안 규칙에 따라, 비정상적인 트래픽이나 의심스러운 활동을 감지하고 차단합니다.
...
> 암호화 과정
- 비대칭키 암호화: 클라이언트와 서버는 RSA 또는 DSA 알고리즘을 사용해 공개키와 비밀키를 교환하여 세션을 설정합니다.
- 대칭키 암호화: 세션이 설정되면, 대칭키(예: AES)를 사용해 이후의 통신을 대칭키로 암호화하여 데이터 전송 속도를 높이고 보안을 유지합니다.
- 데이터 무결성: 데이터가 전송 중에 변조되지 않았음을 보장하기 위해 **HMAC(Hash-based Message Authentication Code)**을 사용해 데이터 무결성을 확인합니다.

## API 호출 시 OAuth 인증 방식을 설명해주세요.
> OAuth(Open Authorization)
- 사용자가 서비스에 로그인하지 않고도 타사 애플리케이션이 사용자 데이터를 접근할 수 있도록 허용하는 인증 프로토콜

> 인증 방식
- 사용자 승인: 사용자가 OAuth 인증을 요청하면, OAuth 제공자(예: Google, Facebook)가 사용자에게 승인을 요청합니다.
- 인가 코드 발급: 사용자가 승인을 허가하면, OAuth 제공자는 애플리케이션에 인가 코드를 발급합니다.
- 토큰 교환: 애플리케이션은 이 인가 코드를 OAuth 제공자에게 보내 액세스 토큰을 교환받습니다.
- API 요청: 애플리케이션은 액세스 토큰을 사용해 API를 호출하고, 요청에 포함된 토큰을 통해 권한이 부여된 데이터에 접근할 수 있습니다.

## SQL 인젝션 공격이란 무엇이며, 이를 방어하는 방법은 무엇인가요?
> SQL 인젝션(SQL Injection)
- 악의적인 사용자가 웹 애플리케이션의 SQL 쿼리에 임의의 SQL 명령어를 삽입하여, 데이터베이스를 조작하거나 민감한 정보를 탈취하는 공격
> 방어 방법
- 파라미터화된 쿼리 사용: 사용자 입력을 쿼리에 직접 포함시키지 않고, Prepared Statement나 파라미터화된 쿼리를 사용하여 데이터베이스 요청을 안전하게 처리합니다.
- ORM 사용: ORM(Object-Relational Mapping) 도구를 사용해 SQL 쿼리 작성 시 보안 위험을 줄입니다.
- 입력 검증: 사용자 입력을 철저히 검증하고, 특수문자나 SQL 예약어가 포함되지 않도록 필터링합니다.
- 최소 권한 원칙 적용: 데이터베이스 사용자에게 최소한의 권한만 부여하여, SQL 인젝션 시에도 피해를 최소화합니다.

## XSS(Cross-Site Scripting)란 무엇이며, 이를 방어하기 위한 방법은 무엇인가요?
> XSS(Cross-Site Scripting)
- 공격자가 악성 스크립트를 웹 페이지에 삽입하여 다른 사용자가 이를 실행하도록 만드는 공격입니다. 이를 통해 사용자의 세션 쿠키, 개인 정보 등을 탈취

> 방어 방법
- 출력 데이터 이스케이프: 사용자 입력을 웹 페이지에 표시할 때는 반드시 HTML 이스케이프 처리를 하여, 스크립트 코드가 실행되지 않도록 합니다.
- Content Security Policy(CSP): 웹 페이지에서 외부 스크립트의 로딩을 제한하는 CSP를 설정하여, 스크립트 실행을 방지합니다.
- 입력 검증: 사용자로부터 입력받은 데이터는 철저히 검증하고, 불필요한 스크립트나 HTML 태그를 필터링합니다.
- HttpOnly 쿠키 사용: HttpOnly 속성을 설정하여, 클라이언트 측 JavaScript가 쿠키에 접근하지 못하도록 하여 세션 탈취를 방지합니다.
...
- TCP 혼잡제어 : 송신자의 전송 속도와 네트워크 처리속도 차이로 인한 문제를 해결하기 위한 기능
- 4가지 단계로 이루어져있음 (느린 시작(Slow Start), 혼잡 회피(Congestion Avoidance), 빠른 재전송(Fast Retransmit), 빠른 회복(Fast Recovery))

> 각 단계별 네트워크 혼잡을 처리하는 과정
- 느린 시작(Slow Start)
  + TCP 연결이 시작될때는 네트워크 상태를 모름. 따라서 느린시작을 통해 CWND 크기를 1로 시작하고, 패킷 전송 이후 ACK 응답을 받으면 지수적으로 증가 1 -> 2 -> 4 -> 8 
  + 혼잡 발생시 CWND(혼잡윈도우)를 1로 줄이는데, 패킷 손실이 발생했을때 데이터를 줄여 혼잡을 완화하는 방식
  + 이후 다시 천천히 전송 속도를 늘려가면서 혼잡이 줄어들었는지 확인 
- 혼잡 회피(Congestion Avoidance)
  + 느린 시작 단계에서 전송 속도를 증가시키다가 임계값에 도달하면 혼잡 회피 단계로 넘어감
  + 혼잡 회피 단게에서는 선형적 증가를 사용하여 전송 속도를 천천히 증가시킴
  + 이때 네트워크가 안정적으로 데이터를 처리할 수 있는지를 탐색
  + 패킷 손실이 감지되면 네트워크 혼잡이 발생했다고 판단
  + CWND 값을 절반으로 줄이고 다시 느린 속도로 데이터를 보내기 시작 
- 빠른 재전송(Fast Retransmit)
  + 타이머가 만료되기 전에 즉시 손실된 패킷을 다시 보냄
- 빠른 복(Fast Recovery)
  + 전송 속도를 천천히 올리면서 네트워크 상태를 모니터링

> 혼잡 윈도우 (Congestion Window, CWND)
- 네트워크 혼잡을 제어하기 위해 한번에 전송할 수 있는 데이터의 최대양을 결정하는 변수

## 클라이언트와 서버 간 데이터 전송 속도를 최적화하기 위한 방법은 무엇인가요?
- 네트워크 대역폭 효율적 사용, CDN 활용, TCP/IP 설정 최적화 등의 전략을 사용  
 
## QoS(Quality of Service)란 무엇이며, 왜 중요하나요?
- 네트워크에서 트래픽의 우선순위를 지정하고 대역폭을 효율적으로 관리하며, 중요한 데이터 전송이 안정적이고 신속하게 이루어질수 있도록 보장하는 기술
- 주요 이점
  + 끊김 없는 서비스 제공
  + 애플리케이션의 성능을 보장함으로써 업무 연속성과 생산성을 높임
  + 네트워크 대역폭을 효율적으로 활용하여 불필요한 트래픽에 의한 성능 저하 방지
  + 네트워크 혼잡 시에도 우선순위가 높은 트래픽이 우선적으로 처리되어 중요한 데이터 손실 방지
