...
- 작업 특성에 따라 스레드 수 계산 공식 사용 ( CPU 바운드 작업, I/O 바운드 작업 )
- CPU 사용률과 스레드 상태 모니터링을 통해 처리 성능이 가장 높은 지점 찾는 방법
- JVM 옵션 설정을 통해 가비지 컬렉션 스레드 수 조절 (-XX:ParallelGCThreads=N(N은 주로 CPU 코어 수에 맞춰 설정)) 

## CPU 바운드(CPU-bound) 작업과 I/O 바운드(I/O-bound) 작업의 특성에 따라 병렬 처리 전략을 어떻게 달리 설정해야 하는가?
> CPU 바운드(CPU-bound) 작업
- CPU 바운드 작업은 계산, 알고리즘 실행 등 CPU를 지속적으로 사용하는 작업 -> 스레드가 대기 없이 CPU에서 계산을 계속 수행하므로 CPU에 집중
- 스레드 수 = 코어 수 동일하게 설정
- 추가 스레드는 컨텍스트 스위칭 오버헤드만 증가되므로 코어당 하나의 스레드만 할당해 사용하는것이 좋음 

> I/O 바운드(I/O-bound) 작업
- I/O 바운드(I/O-bound) 작업은 파일 읽기/쓰기, 네트워크 요청, 디비 쿼리 계산 등 대기 시간이 많이 발생하는 작업 -> 스레드가 CPU를 지속적으로 사용하는 대신에 I/O 작업 중 대기 상태로 전환이 되므로 대기 중에 다른 작업 처리가 가능
- CPU 코어 수보다 많은 스레드 수 설정 -> 대기 시간동안 다른 스레드가 CPU 자원을 사용할수 있도록 함
...
- Stop-the-World (STW) : GC이 시작되면 JVM은 모든 스레드를 일시 중단시키고 메모리 정리가 완료할때까지 스레드를 중단 상태로 유지
- STW로 모든 스레드가 중단되어, 병렬 처리 작업의 흐름이 일시적으로 끊기게 되고 CPU 자원은 대기 시간이 늘어나 처리 성능이 저하
- GC와 애플리케이션이 CPU 자원을 공유하므로, GC가 CPU를 많이 사용할수록 애플리케이션 성능 저하
- 객체 생성과 소멸이 빈번한 병렬 처리 환경에서 GC가 자주 발생하므로 성능 저하
  
> 튜닝 방법
1. 병렬 가비지 컬렉터 선택
- Parallel GC (-XX:+UseParallelGC) : 모든 GC 작업을 병렬로 처리. STW 시간을 줄이는 고성능 GC. 처리량을 중시할때 유용
- G1 GC (-XX:+UseG1GC) : 대용량 힙 메모리에서 STW 시간을 관리하기 위해 설계된 GC. 멀티코어 환경에서 적합. Young, Old 영역을 동시에 병렬로 수집하여 GC의 중단 시간을 줄임 
- ZGC (-XX:+UseZGC)와 Shenandoah GC (-XX:+UseShenandoahGC) : 저지연 GC. 매우 짧은 STW시간을 제공하며 대기시간이 중요한 애플리케이션에 적합.
2. GC 스레드 수 조절
- -XX:ParallelGCThreads=N 및 -XX:ConcGCThreads=N
- 위에 옵션으로 GC 스레드 수 조절 가능
- GC는 내부적으로 스레드를 사용해 병렬로 작업을 수행하는데 GC 스레드 수를 CPU 코어 수의 절반 정도로 설정하면, GC 작업이 병렬로 진행되는 동시에 애플리케이션 스레드도 충분한 CPU 자원을 확보할 수 있어 각자의 수행시간을 개선할수 있음
3. 힙 메모리 크기 조절
- -Xms(초기 힙)와 -Xmx(최대 힙)를 같은 값으로 설정하여 메모리 크기를 고정하면 , 힙 확장 및 축소 과정에서 발생하는 성능 오버헤드 감소
- JVM 힙 크기를 크게 설정하면 애플리케이션이 메모리를 더 많이 사용할 수 있어 객체가 많이 생성되는 작업에서 빈번한 GC를 피할 수 있음 
