...
3. Saga 패턴
- 트랜잭션을 여러개의 작은 트랜잭션(서브 트랜잭션)으로 나누어 관리
- 각 서브 트랜잭션은 독립적으로 커밋되며, 특정 단계에서 실패가 발행하면 보상 트랜잭션을 실행하여 이전에 완료된 작업 취소
4. 보상 트랜잭션
- 분산 트랜잭션에서 일부 작업이 실패했을때, 이미 완료된 이전 작업을 취소하거나 되돌리는 방식으로 일관성을 유지하는 방법
- 주로 saga 패턴과 같이 사용
- 모든 작업에 대해 보상 트랜잭션을 정의해야하므로 구현이 어렵고, 복잡성이 증가
